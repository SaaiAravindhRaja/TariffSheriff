...
public class TariffLookupTool implements ChatbotTool {
    ...
    public ToolResult execute(ToolCall toolCall) {
        ...
        try {
            ...
            // Validate formats
            if (importerIso3.length() != 3 || !importerIso3.matches("[A-Z]{3}")) {
                return ToolResult.error(getName(), 
            ...
            if (originIso3.length() != 3 || !originIso3.matches("[A-Z]{3}")) {
                return ToolResult.error(getName(), 
    ...
    private String formatRateOption(com.tariffsheriff.backend.tariff.dto.TariffRateOptionDto rate) {
        ...
        if (rate.nonAdValorem()) {
            ...
            formatted.append(rate.nonAdValoremText() != null ? rate.nonAdValoremText() : "Non ad valorem charge");
        }
