...
class TariffCalculationRepositoryGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.tariffcalculation.repository.TariffCalculationRepository", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
