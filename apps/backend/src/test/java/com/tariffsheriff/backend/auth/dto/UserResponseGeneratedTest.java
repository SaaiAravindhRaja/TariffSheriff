...
class UserResponseGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.auth.dto.UserResponse", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
