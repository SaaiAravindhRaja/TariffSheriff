...
class SaveTariffCalculationRequestGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.tariffcalculation.dto.SaveTariffCalculationRequest", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
