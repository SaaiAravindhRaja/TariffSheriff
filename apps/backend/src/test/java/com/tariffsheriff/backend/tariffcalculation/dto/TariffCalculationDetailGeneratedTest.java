...
class TariffCalculationDetailGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.tariffcalculation.dto.TariffCalculationDetail", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
