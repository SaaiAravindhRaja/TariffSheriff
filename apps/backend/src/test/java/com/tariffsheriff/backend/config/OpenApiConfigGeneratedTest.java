...
class OpenApiConfigGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.config.OpenApiConfig", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
