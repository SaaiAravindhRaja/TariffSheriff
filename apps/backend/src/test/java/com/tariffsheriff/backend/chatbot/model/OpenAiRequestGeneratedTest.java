...
class OpenAiRequestGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.chatbot.model.OpenAiRequest", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
