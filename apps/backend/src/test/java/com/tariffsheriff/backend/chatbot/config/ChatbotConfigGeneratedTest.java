...
class ChatbotConfigGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.chatbot.config.ChatbotConfig", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
