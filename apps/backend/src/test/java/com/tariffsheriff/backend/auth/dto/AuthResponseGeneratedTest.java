...
class AuthResponseGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.auth.dto.AuthResponse", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
