...
class ErrorResponseGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.web.error.ErrorResponse", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
