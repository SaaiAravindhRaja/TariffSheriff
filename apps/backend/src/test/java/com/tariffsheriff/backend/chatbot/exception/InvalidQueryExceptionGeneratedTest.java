...
class InvalidQueryExceptionGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.chatbot.exception.InvalidQueryException", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
