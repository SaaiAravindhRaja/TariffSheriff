...
class UserControllerGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.auth.controller.UserController", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
