...
class HsProductRepositoryGeneratedTest {

    @Test
    void smoke_loadsClass_and_optionalInstantiate() throws Exception {
        Class<?> cls = Class.forName("com.tariffsheriff.backend.tariff.repository.HsProductRepository", false, Thread.currentThread().getContextClassLoader());
        assertNotNull(cls);
        try {
